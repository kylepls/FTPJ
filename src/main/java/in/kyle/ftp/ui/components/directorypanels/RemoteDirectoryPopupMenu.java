...
public class RemoteDirectoryPopupMenu extends DirectoryPanePopupMenu {
    ...
    public RemoteDirectoryPopupMenu(FTPJ ftpj, JList list) {
        ...
        download.addActionListener(e -> {
            try {
                ftpj.getProtocol().download(getSelectedFakeFile().getRemotePath(), new File(ftpj.getFtpFrame().getLocalDirectory()
                        .getSelectedDirectory(), getSelectedFakeFile().getName()));
